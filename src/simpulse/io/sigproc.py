...
Patched for Python3 bytes/str handling by OA Johnson
"""
...
def sigproc_sex2deg(x):
    """
    Computes a decimal number in degrees from a sigproc number in the form
    ddmmss.s
    """
    sign = 1.0
    if x < 0:
...
def unpack(hdr, param, struct_format):
    """Unpack a numeric value from the header."""
    if isinstance(param, str):
        param_bytes = param.encode("ascii")
    else:
        param_bytes = param

    idx = hdr.find(param_bytes)
    if idx < 0:
        # warnings.warn('Could not find parameter {}'.format(param))
        return None

    idx += len(param_bytes)  # idx is the start of the data after the key
...
def unpack_str(hdr, param):
    """Unpack a string value from the header."""
    if isinstance(param, str):
        param_bytes = param.encode("ascii")
    else:
        param_bytes = param
...
def write_str(f, s):
    """Write a SIGPROC-style string (len + bytes)."""
    if isinstance(s, str):
        s_bytes = s.encode("utf8")
    else:
        s_bytes = s
    n = struct.pack("i", len(s_bytes))
    f.write(n)
    f.write(s_bytes)
...
class SigprocFile(object):
    def __init__(self, filename, mode="r", header=None):
        ...
        if "src_raj" in self.header and self.header["src_raj"] is not None:
            self.src_raj_deg = sigproc_sex2deg(self.header["src_raj"]) * 15.0

        if "src_dej" in self.header and self.header["src_dej"] is not None:
    ...
    def _write_header(self, header):
        ...
        for k, v in header.items():
            ...
            if k in STRING_PARAMS:
                ...
                write_str(f, k)
                write(f, int(v), INT_FORMAT)
            elif k in DOUBLE_PARAMS:
                write_str(f, k)
                write(f, float(v), DOUBLE_FORMAT)
            else:
                # Unknown key – skip
    ...
    def _read_header(self):
        fin = self.fin
        fin.seek(0)
        hdr = fin.read(HEADER_LENGTH)  # bytes
        start_idx = hdr.find(b"HEADER_START")
        end_idx = hdr.find(b"HEADER_END")

        if start_idx < 0 or end_idx < 0:
            raise RuntimeError("Could not find HEADER_START/HEADER_END in file header")

        hdr_section = hdr[start_idx:end_idx]
        self.data_start_idx = end_idx + len(b"HEADER_END")
        self.seek_data()

        header = {}
        self.header = header
        self.hdr = hdr_section

        for p in STRING_PARAMS:
            header[p] = unpack_str(hdr_section, p)

        for p in INT_PARAMS:
            val = unpack(hdr_section, p, INT_FORMAT)
            header[p] = int(val) if val is not None else None

        for p in DOUBLE_PARAMS:
            val = unpack(hdr_section, p, DOUBLE_FORMAT)
            header[p] = float(val) if val is not None else None
        ...
        if getattr(self, "nifs", None) is None:
            self.nifs = 1
        if getattr(self, "nchans", None) is None:
            raise ValueError("Header missing 'nchans'")
        if getattr(self, "nbits", None) is None:
            raise ValueError("Header missing 'nbits'")
        ...
        self.bytes_per_element = int(self.nifs * self.nchans * self.nbits // 8)
        self.file_size_elements = int(self.data_size_bytes // self.bytes_per_element)

        if getattr(self, "nsamples", None) is None:
    ...
    def seek_sample(self, sampnum):
        self.fin.seek(
            self.data_start_idx + sampnum * self.nifs * self.nchans * self.nbits // 8
        )
    ...
    def get_data(self, time_slice, chanindex=0, ifindex=0):
        ...
        num_samples = int(time_end - time_start)
        num_elements = int(num_samples * self.nchans)
        num_dtypes = int(num_elements // samps_per_element)
        ...
        # nbits < 8 not handled here
        num_bytes = int(num_samples * self.nchans * self.nbits // 8)

        if num_bytes + self.data_start_idx > self.file_size_bytes:
            raise ValueError(
                "Requested data off the end of the file. File size: %d. num_bytes: %d. data_start_idx: %d"
                % (self.file_size_bytes, num_bytes, self.data_start_idx)
            )
...
def _main():
    ...
    parser.set_description("SIGPROC header dump")
    parser.add_option(
        "-v",
        "--verbose",
        dest="verbose",
    ...
    for filename in args:
        ...
        fin.print_header()
        if hasattr(fin, "src_raj_deg") and hasattr(fin, "src_dej_deg"):
